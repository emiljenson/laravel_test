...
class ExampleTest extends TestCase
{
    ...
    public function testFutureEvents()
    {
        ...
        $response->assertStatus(200)
            ...
            ->assertJsonPath('0.future_workshops.0.name', 'The new Eloquent - load more with less')
            ->assertJsonPath('0.future_workshops.1.name', 'AutoEx - handles exceptions 100% automatic')
            ->assertJsonPath('1.name', 'React convention ' . $dateFuture->year)
            ->assertJsonPath('1.future_workshops.0.name', '#NoClass pure functional programming')
            ->assertJsonPath('1.future_workshops.1.name', 'Navigating the function jungle');
    }
